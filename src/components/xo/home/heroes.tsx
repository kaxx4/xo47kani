    ...
    <section style={{ background: "var(--milk)", minHeight: "100svh", display: "flex", flexDirection: "column", paddingTop: 92, overflow: "hidden" }}>
      ...
      <div className="wrap-wide" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", paddingTop: 18, paddingBottom: 28 }}>
        <h1 className="display" style={{ fontSize: "clamp(2.9rem, 7.4vw, 6.8rem)", lineHeight: 0.92, marginBottom: "clamp(6px,1.4vw,18px)" }}>
          <span className="rise rise-1">Cut for</span>{" "}
          <span className="italic rise rise-2" style={{ color: "var(--clay)" }}>you.</span>
        </h1>
        <Reveal delay={1}>
          <p className="eyebrow" style={{ marginBottom: "clamp(14px,2vw,26px)" }}>You feel a good suit before anyone notices it</p>
        </Reveal>
        <div className="zoom clip" data-parallax="0.04" style={{ height: "min(58vh, 540px)", aspectRatio: "3/4" }}>
...
export function HeroNoir() {
  return (
    <section style={{ background: "var(--ink)", color: "var(--on-dark)", minHeight: "100svh", display: "flex", alignItems: "center", paddingTop: 96, overflow: "hidden", position: "relative" }}>
        ...
        <div>
          ...
          <Reveal delay={1}>
            <h1 className="display" style={{ fontSize: "clamp(3.6rem,11vw,11rem)", lineHeight: 0.86, marginBottom: 36 }}>
              <span className="rise rise-1">Made by</span><br /><span className="italic rise rise-2" style={{ color: "var(--amber-2)" }}>hand.</span>
            </h1>
          </Reveal>
          <Reveal delay={2}>
            <p className="lede" style={{ color: "var(--on-dark-mut)", maxWidth: "38ch", marginBottom: 44 }}>
              Every suit we make is cut for one person — you. Quietly confident, never loud. People notice; they&apos;re just not sure why.
            </p>
          </Reveal>
          <Reveal delay={3}>
      ...
      <style>{`@media(min-width:768px){.noir-hero{grid-template-columns:1.05fr 0.8fr !important;gap:clamp(40px,5vw,70px) !important}}`}</style>
    </section>
  );
}
    ...
    <section style={{ background: "var(--milk)", color: "var(--ink)", minHeight: "100svh", display: "flex", flexDirection: "column", paddingTop: 96, overflow: "hidden" }}>
      <div className="wrap-wide" style={{ width: "100%" }}>
        <Reveal>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", borderBottom: "1px solid var(--line)", paddingBottom: 16 }}>
            <span className="over">Bespoke Menswear — New Delhi</span>
            <span className="over" style={{ color: "var(--forest)" }}>Made by hand · Since 2020</span>
          </div>
        </Reveal>
      </div>
        ...
        <div>
          <Reveal delay={1}>
            <h1 className="display" style={{ fontSize: "clamp(3.4rem,11vw,12rem)", lineHeight: 0.82, margin: "clamp(20px,4vw,40px) 0 34px" }}>
              <span className="rise rise-1">Built to</span><br /><span className="italic rise rise-2" style={{ color: "var(--forest)" }}>last.</span>
            </h1>
          </Reveal>
          <Reveal delay={2}>
            <p className="lede" style={{ maxWidth: "40ch", marginBottom: 40 }}>
              We start with the best cloth we can find, then cut it to suit you — your build, your life, the way you actually dress.
            </p>
          </Reveal>
          <Reveal delay={3}>
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
      ...
      <style>{`@media(min-width:768px){.earthen-hero{grid-template-columns:1fr 0.92fr !important;gap:clamp(40px,5vw,64px) !important}}`}</style>
    </section>
  );
}
